...
{
    public class OfficeMessageConsumer : BackgroundService, IDisposable
    {
        #region Var
        private readonly ILogger<OfficeMessageConsumer> _logger;
        ...
        public OfficeMessageConsumer(IOptions<RabbitMQCredential> rabbitMqSetting, IOptions<ConnectionModel> connectionString, IServiceProvider serviceProvider, ILogger<OfficeMessageConsumer> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            StartConsuming(RabbitMQQueues.Office, stoppingToken);
        ...
        private async Task<bool> ProcessMessageAsync(string message)
        {
            try
            {
                ...
                var obj = JsonConvert.DeserializeObject<CommonOffice>(message);
                if (obj != null)
                {
                    var acc_Status = await new OfficeService(_connectionString.AccountConnection).Add(obj);
                    if (acc_Status == "")
                        _logger.LogInformation("Office Add in Account Module Successfully.");
                    else _logger.LogWarning(acc_Status);

                    var mf_Status = await new OfficeService(_connectionString.MFConnection).Add(obj);
                    if (mf_Status == "")
                        _logger.LogInformation("Office Add in MF Module Successfully");
                    else _logger.LogWarning(mf_Status);

                    var fxa_Status = await new OfficeService(_connectionString.FixedAssetConnection).Add(obj);
                    if (fxa_Status == "")
                        _logger.LogInformation("Office Add in MF Module Successfully");
                    else _logger.LogWarning(fxa_Status);


                    var proj_Status = await new OfficeService(_connectionString.ProjectConnection).Add(obj);
                    if (proj_Status == "")
                        _logger.LogInformation("Office Add in Project Module Successfully.");
