...
{
    public class GeoLocationMessageConsumer : BackgroundService, IDisposable
    {
        #region Var
        private readonly ILogger<GeoLocationMessageConsumer> _logger;
        ...
        public GeoLocationMessageConsumer(IOptions<RabbitMQCredential> rabbitMqSetting, IOptions<ConnectionModel> connectionString, IServiceProvider serviceProvider, ILogger<GeoLocationMessageConsumer> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            StartConsuming(RabbitMQQueues.GeoLocation, stoppingToken);
        ...
        private async Task<bool> ProcessMessageAsync(string message)
        {
            try
            {
                ...
                var obj = JsonConvert.DeserializeObject<CommonGeoLocation>(message);
                if (obj != null)
                {
                   

                    var mf_Status = await new GeoLOcationService(_connectionString.MFConnection).Add(obj);
                    if (mf_Status == "")
                        _logger.LogInformation("GeoLocation Add in MF Module Successfully");
                    else _logger.LogWarning(mf_Status);

                    var proj_Status = await new GeoLOcationService(_connectionString.ProjectConnection).Add(obj);
                    if (proj_Status == "")
