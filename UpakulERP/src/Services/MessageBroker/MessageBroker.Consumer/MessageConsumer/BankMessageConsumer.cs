...
{
    public class BankMessageConsumer : BackgroundService, IDisposable
    {
        #region Var
        private readonly ILogger<BankMessageConsumer> _logger;
        ...
        public BankMessageConsumer(IOptions<RabbitMQCredential> rabbitMqSetting, IOptions<ConnectionModel> connectionString, IServiceProvider serviceProvider, ILogger<BankMessageConsumer> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            StartConsuming(RabbitMQQueues.Bank, stoppingToken);
        ...
        private async Task<bool> ProcessMessageAsync(string message)
        {
            try
            {
                ...
                var obj = JsonConvert.DeserializeObject<CommonBank>(message);
                if (obj != null)
                {
                    var mf_Status = await new BankService(_connectionString.MFConnection).Add(obj);
                    if (mf_Status == "")
                        _logger.LogInformation("Bank Add in MF Module Successfully");
                    else _logger.LogWarning(mf_Status);
                    
                    var acc_Status = await new BankService(_connectionString.AccountConnection).Add(obj);
                    if (acc_Status == "")
                        _logger.LogInformation("Bank Add in Account Module Successfully.");
                    else _logger.LogWarning(acc_Status);
                    
                    var proj_Status = await new BankService(_connectionString.ProjectConnection).Add(obj);
                    if (proj_Status == "")
                        _logger.LogInformation("Bank Add in Project Module Successfully.");
