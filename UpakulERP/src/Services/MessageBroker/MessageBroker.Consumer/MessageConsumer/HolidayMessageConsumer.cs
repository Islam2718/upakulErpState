...
{
    public class HolidayMessageConsumer : BackgroundService, IDisposable
    {
        #region Var
        private readonly ILogger<HolidayMessageConsumer> _logger;
        ...
        public HolidayMessageConsumer(IOptions<RabbitMQCredential> rabbitMqSetting, IOptions<ConnectionModel> connectionString, IServiceProvider serviceProvider, ILogger<HolidayMessageConsumer> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            StartConsuming(RabbitMQQueues.Holiday, stoppingToken);
        ...
        private async Task<bool> ProcessMessageAsync(string message)
        {
            try
            {
                ...
                var obj = JsonConvert.DeserializeObject<CommonHoliday>(message);
                if (obj != null)
                {
                    //var acc_Status = await new HolidayService(_connectionString.AccountConnection).Add(obj);
                    //if (acc_Status == "")
                    //    _logger.LogInformation("Holiday Add in Account Module Successfully.");
                    //else _logger.LogWarning(acc_Status);

                    var mf_Status = await new HolidayService(_connectionString.MFConnection).Add(obj);
                    if (mf_Status == "")
                        _logger.LogInformation("Holiday Add in MF Module Successfully");
                    else _logger.LogWarning(mf_Status);


                    var proj_Status = await new HolidayService(_connectionString.ProjectConnection).Add(obj);
                    if (proj_Status == "")
                        _logger.LogInformation("Holiday Add in Project Module Successfully.");
