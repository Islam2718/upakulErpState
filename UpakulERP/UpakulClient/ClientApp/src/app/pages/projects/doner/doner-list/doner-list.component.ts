...
import { DonerService } from '../../../../services/Projects/doner.service'; // Adjust path if needed
...
@Component({
  ...
  styleUrl: './doner-list.component.css'
})

export class DonerListComponent implements OnInit, OnDestroy {
  ...
  edit(editId: number) {
    this.router.navigate(['/doner/edit'], { state: { editId: editId } });
  }
  ...
  loadList() {
    ...
    this.apiService.getDoners(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortColumn, this.sortDirection)
      .pipe(
        tap(response => {
  ...
  navigateToCreate() {
    this.router.navigate(['pr/doner/doner']);
  }
