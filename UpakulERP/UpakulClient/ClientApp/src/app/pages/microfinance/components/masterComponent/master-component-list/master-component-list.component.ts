...
@Component({
  selector: 'app-master-component-list',
  imports: [CommonModule, ReactiveFormsModule,ConfirmModalComponent],
  standalone: true,
  templateUrl: './master-component-list.component.html',
  styleUrl: './master-component-list.component.css'
})
export class MasterComponentListComponent implements OnInit, OnDestroy {
  qry: string | null = null;
  //countries: Country[] = [];
  dataList: MasterComponent[] = [];
  page = 1;
  pageSize = 10;
  totalPages = 1;
  totalRecords = 0;
  ...
  loadList() {
    ...
    this.apiService.getMasterComponents(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortColumn, this.sortDirection)
      .pipe(
        tap(response => {
...
edit(editId: number) {
    this.router.navigate(['master-component-form/edit'], { state: { editId: editId } });
  }
  ...
  onDeleteConfirmed() {
    ...
    this.apiService.deleteMasterComponent(this.deleteIdToConfirm).subscribe({
      next: (response) => {
  ...
  navigateToCreate() {
    this.router.navigate(['mf/masterComponent/master-component-form']);
  }
