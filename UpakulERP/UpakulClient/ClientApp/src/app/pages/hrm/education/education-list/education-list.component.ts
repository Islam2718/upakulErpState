...
import { EducationService } from '../../../../services/hrm/education/education.service'; // Adjust path if needed
...
export class EducationListComponent implements OnInit, OnDestroy {
  ...
  sortDirection = 'asc';
  sortOrder: string = 'EducationId asc'
  ...
  editEducation(educationId: number) {
    ...
    this.router.navigate(['/education/edit'], { state: { educationId: educationId } });
  }
...
onDeleteConfirmed() {
    ...
    this.educationService.deleteEducation(this.deleteIdToConfirm).subscribe({
      next: (response) => {
        if (response.type === 'warning') {
          this.toastr.warning(response.message, 'Warning');
        } else if (response.type === 'strongerror') {
  ...
  loadList() {
    ...
    this.educationService.getEducationList(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortOrder)
      .pipe(
        tap(response => {
          //  console.log("API Response: ", response);
          if (response && response.totalRecords !== undefined) {
            this.educations = response.listData;
  ...
  navigateToCreate() {
    this.router.navigate(['hr/hrm/education']);
  }
