...
export class OfficeListComponent implements OnInit, OnDestroy {
  ...
  edit(editId: number) {
    ...
    // console.log(editId);
    this.router.navigate(['/office/edit'], { state: { editId: editId } });
  }
  ...
  loadOffices() {
    ...
    this.officeService
      .GetOffices(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortOrder)
      .pipe(
        tap(response => {
          //  console.log("API Response: ", response);
          if (response && response.totalRecords !== undefined) {
            this.offices = response.listData;
  ...
  onDeleteConfirmed() {
    ...
    this.officeService.deleteData(this.deleteIdToConfirm).subscribe({
      next: (response) => {
  ...
  navigateToOfficeCreate() {
    this.router.navigate(['gs/global/office']);
  }
}
