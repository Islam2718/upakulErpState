...
import { BankBranchService } from '../../../../services/Global/bankbranch/bank-branch.service'; // Adjust path if needed
...
@Component({
  selector: 'app-bankbranch-list',
  imports: [CommonModule, ReactiveFormsModule, ConfirmModalComponent],
  standalone: true,
  templateUrl: './bankbranch-list.component.html',
  styleUrl: './bankbranch-list.component.css'
})
export class BankbranchListComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, // FormBuilder to easily create the form group
    private router: Router, private http: HttpClient, private BankBranchService: BankBranchService,private toastr: ToastrService) {
  ...
  loadBankBranch() {
    this.BankBranchService
      .getBankBranches(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortOrder)
      .pipe(
        tap(response => {
          ///console.log("API Response: ", response);
          if (response && response.totalRecords !== undefined) {
            this.bankbranches = response.listData;
  ...
  navigateToCreate() {
    this.router.navigate(['gs/global/bankbranchform']);
  }
  ...
  edit(editId: number) {
    console.log('In edit');
    console.log(editId);
    this.router.navigate(['/bankbranchform/edit'], { state: { editId: editId } });
  }
  ...
  onDeleteConfirmed() {
    ...
    this.BankBranchService.deleteBankBranch(this.deleteIdToConfirm).subscribe({
      next: (response) => {
        ...
        this.loadBankBranch(); // Reload list
      },
      error: (error) => {
