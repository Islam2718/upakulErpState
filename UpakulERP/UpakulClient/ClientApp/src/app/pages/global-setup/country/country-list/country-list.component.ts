...
import { CountryService } from '../../../../services/Global/country/country.service'; // Adjust path if needed
...
export class CountryListComponent implements OnInit, OnDestroy {
  ...
  sortDirection = 'asc';
  sortOrder: string = 'CountryName asc'
  ...
  edit(editId: number) {
    this.router.navigate(['/country/edit'], { state: { editId: editId } });
  }
  ...
  onDeleteConfirmed() {
    ...
    this.apiService.deleteCountry(this.deleteIdToConfirm).subscribe({
      next: (response) => {
  ...
  loadList() {    
    this.apiService.getCountries(this.page, this.pageSize, this.searchForm.get('searchTerm')?.value, this.sortOrder)
      .pipe(
        tap(response => {          
  ...
  navigateToCreate() {
    this.router.navigate(['gs/country/country']);
  }
